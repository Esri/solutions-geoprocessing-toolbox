...
# GenerateIntermediateLayersLinearObstacles
...
def FeatureClassesFromWorkspace(inputFD):
    ''' get a list of feature classes in a workspace '''
    arcpy.env.workspace = os.path.dirname(inputFD)
    tdsFeatureClasses = arcpy.ListFeatureClasses("*", "Line",
                                                 os.path.basename(inputFD))
...
def main():
    ...
    try:
        ...
        target = os.path.join(inputMAOTWorkspace, "CombinedLinearObstacleFeature")
