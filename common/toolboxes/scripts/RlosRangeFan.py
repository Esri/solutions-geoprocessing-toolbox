...
if inFeature == '#' or not inFeature:
    inFeature = "in_memory\\{F0791C9D-337D-4937-817D-5ACD9CF290CC}" # provide a default value if unspecified
input_surface = arcpy.GetParameterAsText(1)
tower = arcpy.GetParameterAsText(2)
observation_height = arcpy.GetParameterAsText(3)
weaponModel = arcpy.GetParameterAsText(4)
...
outWorkspace = arcpy.GetParameterAsText(8)
# The name of the featureclass in which the features should be stored
outFeatureClassName = arcpy.GetParameterAsText(9)
...
try:
    ...
    arcpy.MinimumBoundingGeometry_management(inFeature,observers_mbg,"RECTANGLE_BY_AREA") 
